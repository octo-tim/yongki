...
export default async function MeetingsPage() {
  const [clients, factories, projects, meetings] = await Promise.all([
  ...
  return (
    <div className="space-y-5 p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">내부회의 · 외부회의(업체/공장) 기록 · 첨부파일</p>
      </div>
      <Card>
        <CardContent className="p-4">
          <MeetingPanel clients={clients} factories={factories} projects={projects} meetings={meetings as any} />
        </CardContent>
      </Card>
    </div>
  );
