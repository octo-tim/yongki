...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { type, amount, receivedAt, method, account, memo } = await req.json();
  ...
  const pay = await prisma.payment.create({
    data: { projectId: params.id, type, amount: amt, receivedAt: new Date(receivedAt), method: method || null, account: account || null, memo: memo || null },
  });
