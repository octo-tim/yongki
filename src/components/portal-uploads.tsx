...
import { FileCheckButton } from "@/components/file-check-button";
...
export async function PortalUploads() {
  const files = await prisma.clientPortalRequest.findMany({
    where: { fileName: { not: null }, fileCheckedAt: null } as any,
    ...
    <section className="space-y-2">
      ...
      <Card>
        <CardContent className="p-0">
          {files.length === 0 ? (
            <p className="px-4 py-6 text-center text-sm text-muted-foreground">확인 대기 중인 고객 업로드 파일이 없습니다.</p>
          ) : (
            <div className="divide-y">
              {files.map((f) => (
                <div key={f.id} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <FileCheckButton requestId={f.id} />
